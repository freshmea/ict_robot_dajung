...
class Calculator(Node):
    def __init__(self):
        ...
        self.qos_profile = QoSProfile(history=QoSHistoryPolicy.KEEP_LAST,
                                      ...
                                      depth = 10)
        self.argument_subscriber = self.create_subscription(ArithmethicArgument, 'arithmetic_argument', self.get_arithmetic_argument, self.qos_profile)
        self.arithmethic_service_server = self.create_service(ArithmeticOperator, 'arithmetic_operator', self.get_arthmetic_operator, callback_group=ReentrantCallbackGroup())
    ...
    def get_arthmetic_operator(self, request, response):
        ...
        self.argument_formula = f'{self.argument_a} {self.argument_symbol[self.argument_operator-1]} {self.argument_b} = {self.argument_result}'
        self.get_logger().info(f'Service called {self.argument_a} {self.argument_symbol[self.argument_operator-1]} {self.argument_b} = {self.argument_result}')
        return response
